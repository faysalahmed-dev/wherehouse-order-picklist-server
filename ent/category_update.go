...
func (cu *CategoryUpdate) SetNillableUserID(id *uuid.UUID) *CategoryUpdate {
	if id != nil {
		cu = cu.SetUserID(*id)
	}
	return cu
...
func (cu *CategoryUpdate) check() error {
	...
	if v, ok := cu.mutation.Value(); ok {
		if err := category.ValueValidator(v); err != nil {
			return &ValidationError{Name: "value", err: fmt.Errorf(`ent: validator failed for field "Category.value": %w`, err)}
		}
	}
	return nil
}
...
// SetNillableUserID sets the "user" edge to the User entity by ID if the given value is not nil.
func (cuo *CategoryUpdateOne) SetNillableUserID(id *uuid.UUID) *CategoryUpdateOne {
	if id != nil {
