...
func (uq *UserQuery) QuerySubCategories() *SubCategoryQuery {
	query := (&SubCategoryClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithSubCategories(opts ...func(*SubCategoryQuery)) *UserQuery {
	query := (&SubCategoryClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSubCategories = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSubCategories; query != nil {
		if err := uq.loadSubCategories(ctx, query, nodes,
			func(n *User) { n.Edges.SubCategories = []*SubCategory{} },
			func(n *User, e *SubCategory) { n.Edges.SubCategories = append(n.Edges.SubCategories, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadSubCategories(ctx context.Context, query *SubCategoryQuery, nodes []*User, init func(*User), assign func(*User, *SubCategory)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.SubCategory(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SubCategoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_sub_categories
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_sub_categories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_sub_categories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
