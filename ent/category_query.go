...
func (cq *CategoryQuery) loadSubCategories(ctx context.Context, query *SubCategoryQuery, nodes []*Category, init func(*Category), assign func(*Category, *SubCategory)) error {
	...
	for _, n := range neighbors {
		fk := n.category_sub_categories
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_sub_categories" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_sub_categories" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
