...
func (scq *SubCategoryQuery) QueryUser() *UserQuery {
...
func (scq *SubCategoryQuery) WithUser(opts ...func(*UserQuery)) *SubCategoryQuery {
	query := (&UserClient{config: scq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (scq *SubCategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SubCategory, error) {
	...
	if scq.withCategory != nil || scq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := scq.withUser; query != nil {
		if err := scq.loadUser(ctx, query, nodes, nil,
			func(n *SubCategory, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (scq *SubCategoryQuery) loadCategory(ctx context.Context, query *CategoryQuery, nodes []*SubCategory, init func(*SubCategory), assign func(*SubCategory, *Category)) error {
	...
	nodeids := make(map[uuid.UUID][]*SubCategory)
	for i := range nodes {
		if nodes[i].category_sub_categories == nil {
			continue
		}
		fk := *nodes[i].category_sub_categories
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_sub_categories" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (scq *SubCategoryQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*SubCategory, init func(*SubCategory), assign func(*SubCategory, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*SubCategory)
	for i := range nodes {
		if nodes[i].user_sub_categories == nil {
			continue
		}
		fk := *nodes[i].user_sub_categories
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_sub_categories" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
